...
8\. 커밋을 만들 때마다, 그 커밋 해시로 배포(push)하는 명령어를 함께 준다.

&#x20;  형식: `git push origin <해시>:main` (Vercel이 origin/main을 자동 배포하므로

&#x20;  이 명령이 곧 "이 커밋까지만 운영 반영"이다). 여러 커밋이 쌓였으면 각

&#x20;  단계별 명령을 순서대로 나열한다. push는 내가 직접 실행한다 — 대신 돌리지 않는다.
