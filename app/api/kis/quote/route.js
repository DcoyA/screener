...
async function fetchPrice({ baseUrl, token, appKey, appSecret, code }) {
...
async function fetchMinute({ baseUrl, token, appKey, appSecret, code }) {
  const apiUrl = new URL(
  ...
  if (!res.ok || data.rt_cd !== "0") {
    console.error("KIS minute error:", data);
...
export async function GET(request) {
  try {
    ...
    const minuteData = await fetchMinute({
      baseUrl,
      token,
      appKey,
      appSecret,
    ...
    console.error("KIS quote API error:", error);

    return NextResponse.json(
      {
        ok: false,
        error: error.message || "KIS 통합 시세 조회 실패",
      },
      { status: 500 }
    );
  }
