...
function buildDecisionReasons(stock, riskItem, scores, decision) {
  ...
  if (decision === "매수 후보") {
    reasons.push(`기존 랭킹 이후 조건을 한 번 더 걸렀을 때도 통과한 종목입니다.`);
  } else if (decision === "관찰") {
    reasons.push(`기존 랭킹에서는 괜찮지만 실전 진입 전 한 번 더 관찰이 필요한 종목입니다.`);
  } else {
    reasons.push(`랭킹 상위권일 수는 있어도 지금 바로 매수 대상으로 보긴 어렵습니다.`);
  }
          ...
          {items.map((item) => (
            <article className="pickCard" key={`${title}-${item.code}`}>
              ...
              <div className="reasonCard">
                <span className="reasonLabel">판정 이유</span>
...
export default function FinalPicksPage() {
  ...
  const currentItems =
    selectedGroup === "watch" ? finalPicks.watch : selectedGroup === "exclude" ? finalPicks.exclude : finalPicks.buy;
    ...
    <main className="container">
      ...
      <section className="pageHero">
        ...
        <div className="heroInfoGrid">
          <div className="heroInfoCard good">
            <span>매수 후보</span>
            <strong>{finalPicks.buy.length}</strong>
          </div>
          <div className="heroInfoCard mid">
            <span>관찰 후보</span>
            <strong>{finalPicks.watch.length}</strong>
          </div>
          <div className="heroInfoCard warn">
            <span>제외 후보</span>
            <strong>{finalPicks.exclude.length}</strong>
          </div>
        </div>
      </section>
      ...
      <section className="switchSection">
        <div className="switchCard">
          <div className="tabRow">
            <button
              type="button"
              className={`tabBtn ${selectedGroup === "buy" ? "active" : ""}`}
            ...
            <button
              type="button"
              className={`tabBtn ${selectedGroup === "watch" ? "active" : ""}`}
              onClick={() => setSelectedGroup("watch")}
            >
              관찰 후보
            </button>
            <button
              type="button"
              className={`tabBtn ${selectedGroup === "exclude" ? "active" : ""}`}
              onClick={() => setSelectedGroup("exclude")}
            >
              제외 후보
            </button>
          </div>
        ...
        <div className="logicCard">
          ...
          <div className="guideGrid twoCol">
            <div className="guideItem">
              ...
              <span>
                기존 stocks.json + risks.json + 업종/뉴스 플래그 필드만으로 최종 점수를 계산합니다.
                빠르게 붙일 수 있고, 현재 서비스 신뢰 회복용 1차 버전으로 적합합니다.
              </span>
            </div>
            <div className="guideItem">
              <strong>B안: AI 해석 확장</strong>
              <span>
                뉴스/공시 요약과 자연어 판정 설명까지 붙여서 왜 통과/제외됐는지 사람이 읽는 문장으로 더 정교하게 보여줍니다.
                MVP 이후 고도화 단계에 적합합니다.
              </span>
            </div>
          </div>
        </div>
      ...
      <style jsx>{`
        ...
        .explainCard, .switchCard, .logicCard, .pickCard, .emptyBox {
          border: 1px solid #e5e7eb;
          border-radius: 28px;
          padding: 24px;
        ...
        .explainCard h2, .logicCard h2, .sectionTitle {
          margin: 0 0 16px;
          font-size: 1.5rem;
        ...
        .guideItem span { color: #64748b; line-height: 1.7; font-size: 0.95rem; }
        ...
        .riskChip, .decisionChip, .smallBadge {
          display: inline-flex;
          align-items: center;
          justify-content: center;
          padding: 7px 12px;
